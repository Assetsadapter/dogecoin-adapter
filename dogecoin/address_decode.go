...
var (
	DOGE_mainnetAddressP2PKH = addressEncoder.AddressType{"base58", addressEncoder.BTCAlphabet, "doubleSHA256", "h160", 20, []byte{0x1E}, nil}
	DOGE_testnetAddressP2PKH = addressEncoder.AddressType{"base58", addressEncoder.BTCAlphabet, "doubleSHA256", "h160", 20, []byte{0x6F}, nil}
)
...
func (decoder *addressDecoder) PrivateKeyToWIF(priv []byte, isTestnet bool) (string, error) {
	...
	cfg := DOGE_mainnetAddressP2PKH
	if decoder.wm.Config.IsTestNet {
		cfg = DOGE_testnetAddressP2PKH
	}
...
func (decoder *addressDecoder) PublicKeyToAddress(pub []byte, isTestnet bool) (string, error) {
	...
	if decoder.wm.Config.RPCServerType == RPCServerCore {
		log.Info(fmt.Sprintf("new address is : %s", address))
...
func (decoder *addressDecoder) RedeemScriptToAddress(pubs [][]byte, required uint64, isTestnet bool) (string, error) {
	...
	cfg := addressEncoder.DOGE_multiSignAddressP2PKH
	if decoder.wm.Config.IsTestNet {
		cfg = addressEncoder.DOGE_multiSignAddressP2PKH
	}
